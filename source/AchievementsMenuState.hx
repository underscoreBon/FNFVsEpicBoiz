...
class AchievementsMenuState extends MusicBeatState
{
	...
	override function create() {
		...
		resetText = new FlxText(0, 0, 1000, "Press [RESET] To Reset Awards", 12);
		resetText.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		resetText.scrollFactor.set();
		resetText.borderSize = 2.4;
		add(resetText);
		changeSelection();
	...
	override function update(elapsed:Float) {
		...
		if (controls.RESET) {
			if (canreset == 0){
			resetText.text = "Press [RESET] again To confirm";
            canreset = 1;
			}else{
			resetText.text = "RESETTED!";
			FlxG.save.data.achievementsMap = null;
			for (i in 0...Achievements.achievementsStuff.length) {
				Achievements.achievementsMap.set(Achievements.achievementsStuff[i][2], false);
			} //iterations lmaoooo i love iterations!!!!!!!!1
			}
		}
	}
