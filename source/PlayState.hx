...
class PlayState extends MusicBeatState
{
	...
	public static var onehpercentThingy:Array<String> = [];
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['eweek1_nomiss', 'eweek2_nomiss', 'eweek3_nomiss', 'eweek1FC', 'eweek2FC', 'eweek3FC', 'funni', 'chaptercomplete', 'allFC']);
	...
	private function checkForAchievement(achievesToCheck:Array<String> = null):String
	{
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				{
					case 'eweek1_nomiss' | 'eweek2_nomiss' | 'eweek3_nomiss':
						if(isStoryMode && storyPlaylist.length <= 1 && !usedPractice)
						{
							...
							{
								case 'Eweek1':
									if(achievementName == 'eweek1_nomiss')
									{
									unlock = true;
									GameJoltAPI.getTrophy(174565);
									}
								case 'Eweek2':
									if(achievementName == 'eweek2_nomiss')
										{
										unlock = true;
										GameJoltAPI.getTrophy(174566);
										}
								case 'Eweek3':
									if(achievementName == 'eweek1_nomiss')
										{
										unlock = true;
										GameJoltAPI.getTrophy(174567);
										}
							}
						}
					case 'eweek1FC' | 'eweek2FC' | 'eweek3FC':
						if(isStoryMode && campaignMisses + songMisses < 1  && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
						{
							...
							{
								case 'Eweek1':
									if(achievementName == 'eweek1_FC')
									{
									unlock = true;
									GameJoltAPI.getTrophy(174568);
									}
								case 'Eweek2':
									if(achievementName == 'eweek2_FC')
										{
										unlock = true;
										GameJoltAPI.getTrophy(174569);
										}
								case 'Eweek3':
									if(achievementName == 'eweek1_FC')
										{
										unlock = true;
										GameJoltAPI.getTrophy(174570);
										}
							}
						}
				...
				if (songMisses == 69){
				unlock = true;
				GameJoltAPI.getTrophy(174571);
				}
				case 'allFC':
				if (Achievements.isAchievementUnlocked('eweek1FC') && Achievements.isAchievementUnlocked('eweek2FC') && Achievements.isAchievementUnlocked('eweek3FC')){
					unlock = true;
					GameJoltAPI.getTrophy(174610);
				}
				case 'chaptercomplete':
				if (Achievements.isAchievementUnlocked('eweek1_nomiss') && Achievements.isAchievementUnlocked('eweek2_nomiss') && Achievements.isAchievementUnlocked('eweek3_nomiss')){
					unlock = true;
					GameJoltAPI.getTrophy(174611);
				}
				case '100percent':
					onehpercentThingy = [ //all achevements included in thing
						'logged',
						'eweek1_nomiss',
						'eweek2_nomiss',
						'eweek3_nomiss',
						...
						'allFC',
						'chaptercomplete'
					];// I am too lazy to do the the easy way
					for (i in 0...onehpercentThingy.length){
						if (Achievements.isAchievementUnlocked(onehpercentThingy[i])){
							unlock = true;
						}else{
							unlock = false;
						}
